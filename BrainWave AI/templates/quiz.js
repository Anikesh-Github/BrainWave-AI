...
// ---------------------------
// ===========================
// 🔥 MINIMAL ANTI-CHEAT (TAB SWITCH ONLY)
// ===========================
...
document.addEventListener("visibilitychange", () => {
    if (antiCheatActive && document.hidden) {
        cheatCount++;
        alert("⚠️ Cheating detected: Tab Switch");
        if (cheatCount >= 1) {
            chk_ans();   // auto-submit
        }
    }
...
// ---------------------------
// Original quiz.js content (integrated)
// ---------------------------
...
try {
    ...
    window.onload = function () {
        ...
        // If there's a Start button, attach activation to it. Otherwise, start on load.
        const startBtn = document.getElementById('startQuizBtn');
        function activateQuizUIAndAntiCheat() {
            // start anti-cheat only when quiz actually begins
            startAntiCheat();

            // show quiz UI (your original behavior)
            document.getElementById('body').style.display = 'block';
            document.getElementById('step1').style.display = 'block';

            // Load selection for first question
            loadSelection(1);
        }

        if (startBtn) {
            // If Start button exists, bind click to initialize quiz
            startBtn.addEventListener('click', function (e) {
                e.preventDefault();
                // hide start area if you have one (optional)
                try {
                    startBtn.style.display = 'none';
                } catch (er) {}
                // Insert the generated quizzes into DOM
                ...
                // Hide option boxes that don't exist
                for (let index = 1; index <= db['all']; index++) {
                ...
                // Activate UI and anti-cheat
                activateQuizUIAndAntiCheat();
            });
        } else {
            // No start button: fallback to auto-start on load (keeps previous behavior)
            document.getElementById('url-link').setAttribute('onclick', `copy()`)
            document.getElementById('down_link').setAttribute('href', `${db['url']}.json`)
            document.getElementById('stepForm').innerHTML = raw;
            for (let index = 1; index <= db['all']; index++) {
            ...
            // Start anti-cheat and show quiz immediately
            activateQuizUIAndAntiCheat();
        }
